...
func CreateHistoricalOhnoCountersTableIfNotExists(db *sql.DB) {
	...
	createTableQuery := `
		CREATE TABLE IF NOT EXISTS historical_ohno_counters (
	...
	createTriggerQuery := `
		...
		BEGIN
			IF NOT EXISTS (
				...
				BEFORE UPDATE ON historical_ohno_counters
				FOR EACH ROW
	...
	if err != nil {
		log.Fatalf("❌ Error creating trigger for historical_ohno_counters table.\n %s", err)
	}

	log.Println("✅ Ensured historical_ohno_counters table and trigger exist.")
}
